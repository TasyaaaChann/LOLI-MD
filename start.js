...
const { default: makeWASocket, useSingleFileAuthState, DisconnectReason, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, proto } = require("@adiwajshing/baileys-md")
...
browser: ['Loli Bot MD','Safari','1.0.0'],
auth: state,
version: [2, 2204, 13]
})
...
zeroyt7.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Yah Kok Keluar Dari *${metadata.subject}* Mental aman kan kak, Sayonara @${num.split("@")[0]} :(` })
}
}
} catch (err) {
console.log(err)
...
zeroyt7.sendText = (jid, text, quoted = '', options) => zeroyt7.sendMessage(jid, { text: text, ...options }, { quoted })
   ...
   /** Send Button 5 Image
     ...
     * @returns
     */
zeroyt7.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: zeroyt7.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            zeroyt7.relayMessage(jid, template.message, { messageId: template.key.id })
    }
     ...
     * @param {*} options 
     */
    zeroyt7.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            text,
            footer,
            buttons,
        ...
        zeroyt7.sendMessage(jid, buttonMessage, { quoted, ...options })
    }
